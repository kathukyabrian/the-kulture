...
export class TravellerComponent implements OnInit {
  ...
  load(): void { this.loading = true; this.error = ''; this.api.getTravellerContext().subscribe({ next: context => { this.context = context; this.loading = false; this.loadVehicles(); }, error: response => { this.loading = false; this.error = response.status === 403 ? 'Your traveller account is not active.' : 'Could not load your traveller account.'; } }); }
